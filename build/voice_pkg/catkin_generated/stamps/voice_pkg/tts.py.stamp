...
大致流程：
    ...
    消息发布：
        开启asr、tts。tts开启之后，向voice_asr发布1开始。
        当asr有结果之后，向asr_result发布出去，tts接收会
        先向voice_asr发布0,然后进行播报
"""
...
class AI:
    def __init__(self, text):
        ...
        self.result_text = ""   # AI回复结果
        # 更新后的Cookie（请根据实际情况替换）
        self.cookie = "oai-thread-sidebar=%7B%22isOpen%22%3Atrue%7D; _ga=GA1.1.717826682.1763693220; authorized=1; gfsessionid=19th2dn1stnfbfdgn7duk0a0tl10807x; _account=475334ee-3807-4b95-b0aa-2cf6b4e72757; oai-client-auth-info=%7B%22user%22%3A%7B%22name%22%3A%222ho96bm7%7C2026-03-24%2009%3A34%3A14%22%2C%22email%22%3A%222ho96bm7%7C2026-03-24%2009%3A34%3A14%22%2C%22picture%22%3A%22%2Favatars.png%22%2C%22connectionType%22%3A2%2C%22timestamp%22%3A1771937410500%7D%2C%22isOptedOut%22%3Afalse%7D; _ga_9SHBSK2D9J=GS2.1.s1771937410$o31$g0$t1771937410$j60$l0$h0; oai-last-model-config=%7B%22model%22%3A%22gpt-5-2-thinking%22%7D; _dd_s=aid=aa3efa8d-98e9-4d08-bb38-d50b71fa64fd&logs=1&id=3774838e-5001-44ab-8ede-1c683fb1c468&created=1771937407355&expire=1771938427344"
    ...
    def parse_sse_data(self, line):
        """解析SSE数据流中的单行数据，增加严格的类型判断"""
        ...
        # 跳过空数据、结束标记和非JSON字符串（如"v1"）
        if not data_str or data_str == "[DONE]" or data_str in ['"v1"', "v1"]:
            return None
    ...
    def send_message_to_ai(self):
        """发送消息到AI接口，处理流式响应并返回完整回复"""
        url = "https://chatgpt.9e.lv/backend-api/f/conversation"
        ...
        headers = {
            ...
            "Referer": "https://chatgpt.9e.lv/",
            "Origin": "https://chatgpt.9e.lv"
        }
        ...
        payload = {
            ...
            "parent_message_id": session_data["parent_message_id"],
            "model": "gpt-5-2",  # 从抓包信息更新为gpt-5-2
            ...
            "client_contextual_info": {
                "is_dark_mode": False,
                "time_since_loaded": 127,
                "page_height": 914,
                "page_width": 1272,
                "pixel_ratio": 1,
                "screen_height": 1080,
                "screen_width": 1920,
                "app_name": "chatgpt.9e.lv"
            },
        ...
        # 如果已有会话ID，添加到请求体
        if session_data["conversation_id"]:
        ...
        try:
            ...
            if response.status_code != 200:
                raise RuntimeError(f"请求失败，状态码: {response.status_code}, 响应: {response.text[:500]}")
            ...
            for line in response.iter_lines(decode_unicode=True):
                ...
                # 2. 提取助手消息ID（用于后续parent_message_id）
                if data.get("type") == "message_marker" and "message_id" in data:
                    assistant_message_id = data["message_id"]
                elif isinstance(data.get("v"), dict) and "message" in data["v"]:
                    msg_data = data["v"]["message"]
                    if isinstance(msg_data, dict) and msg_data.get("author", {}).get(
                            "role") == "assistant" and msg_data.get("id"):
                        assistant_message_id = msg_data["id"]

                # 3. 处理文本增量更新 - 增加严格的类型判断
                # 情况1: 直接append的格式 {"p": "...", "o": "append", "v": "..."}
                if isinstance(data.get("v"), str) and data.get("o") == "append" and data.get(
                        "p") == "/message/content/parts/0":
                    full_response.append(data.get("v", ""))
                # 情况2: patch数组格式 {"v": [{"p": "...", "o": "append", "v": "..."}]}
                elif isinstance(data.get("v"), list):
                    for patch in data["v"]:
                        if (isinstance(patch, dict) and
                                patch.get("o") == "append" and
                                patch.get("p") == "/message/content/parts/0" and
                                isinstance(patch.get("v"), str)):
                            full_response.append(patch["v"])

            # 更新parent_message_id为助手消息ID，用于下一轮对话
            if assistant_message_id:
                session_data["parent_message_id"] = assistant_message_id
    ...
    def start(self):
        ...
        try:
            global scenario
            self.result_text = self.send_message_to_ai()
            scenario = ""
            ...
            session_data["parent_message_id"] = "client-created-root"
            return ""
...
class Ws_Param ( object ):
    ...
    def __init__(self, APPID, APIKey, APISecret):
        # 三组件
        self.APPID = 'c6f9e3d1'
        self.APIKey = '945c69ddccae092cf01aa5212ed85152'
        self.APISecret = 'YjQ2ODQwMGIwZTVhMTVhMTFjY2Q3M2Y1'
...
def main():
    ...
    # ====================== 旧
    # session_data = {  # ai纠正类中存储会话id。用于确保在同一聊天框、有历史记录
    ...
    # 给GPT的情景说明文本
    scenario = ("你是万能的专家, 我现在要将你集成到我的ros小车机器人上面供用户用语音调用, 用户的输入有两类, "
            "一类是闲聊式, 一类是命令式 (驱动机器人) 。你在接收到输入时, 首先对输入就行常规语法纠正 (语音识别有可能识别不准) ,"
            "然后判断输入类型，如果是简单的问题，我需要你快速响应，涉及比较难的问题可以适当延长思考时间。"
            "1.闲聊："
            "用户有时候会输入一些命令之外的内容, 你回答的时候就简明扼要一点。注：无需指明是什么类型的问题，直接回复即可"
            "2.命令式 (驱动机器人) ："
            "现在我给用户的控制内容就只有导航和前后左右走, 因此只有这两类才属于命令式 (驱动机器人) 。"
            "你在识别到这两类指令时, 首先也是先进行语法纠正, 然后返回'导航点几 (数字) '或者'前 (单个方位词) '即可。"
            "其他任何东西都不能回复。\n 我要问的第一个问题是：")       
