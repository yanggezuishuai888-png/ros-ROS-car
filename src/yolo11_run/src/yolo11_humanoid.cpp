...
void detect_yolo11(const cv::Mat& bgr, std::vector<Object>& objects, ncnn::Net& yolo11)
{
    // ncnn::Net yolo11;
    ...
    // yolo11.load_param("/home/rm/yolo_model/ncnn/best_ncnn_model/result/best_pnnx.ncnn.param");
    // yolo11.load_model("/home/rm/yolo_model/ncnn/best_ncnn_model/result/best_pnnx.ncnn.bin");
...
void draw_objects(const cv::Mat& image, const std::vector<Object>& objects)
{
    ...
    // static const char* class_names[] = {"0", "1", "2", "3", "4", "5", "6", "7"};
    ...
    // cv::waitKey(0);
    // cv::namedWindow("image", cv::WINDOW_NORMAL);   // 关键
    // cv::resizeWindow("image", 800, 600);            // 设置你想要的窗口大小
    // cv::imshow("image", image);
    // cv::waitKey(1);
...
int main(int argc, char** argv)
{
    if (argc != 2)
    {
        fprintf(stderr, "Usage: %s [image|video]\n", argv[0]);
        return -1;
    }
    ...
    if (!image.empty())
    {
        ...
        detect_yolo11(image, objects, yolo11);
